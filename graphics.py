# graphics.py
"""
***Modified version for Minesweeper project***

* I modified the Image class so that the constructor takes width and
height and resizes the underlying PhotoImage.

* I also separated the left and right mouse click functions so that
it the program can differentiate between the two.
...
# Version 3.3 8/8/06
#     Added checkMousePosition method to GraphWin
# Version 3.2.3
...
class GraphWin(tk.Canvas):
    ...
    def getMouse(self):
        """
        Wait for mouse click and return tuple in the form "point, button"
        where 'point' is a Point object representing the click's x and y, and
        'button' is a string representing which button on the mouse was clicked
        """
        self.update()  # flush any prior clicks
        self.mouseX = None
        self.mouseY = None
        self.mouseButton = None
        while self.mouseX == None or self.mouseY == None or self.mouseButton == None:
            self.update()
            if self.isClosed():
    ...
    def checkMousePosition(self):
        ...
        if self.isClosed():
            raise GraphicsError("checkMousePosition in closed window")
        self.update()
