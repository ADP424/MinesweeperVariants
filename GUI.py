...
def create_tile_board(minesweeper_board: MinesweeperBoard) -> list:
    ...
    tile_size = min((WINDOW_HEIGHT - WINDOW_BORDERS) / minesweeper_board.board_height, (WINDOW_WIDTH - WINDOW_BORDERS) / minesweeper_board.board_width)
...
def get_value_images(row: int, col: int, tile_size: float, tile_board: list, minesweeper_board: MinesweeperBoard):
    """
    ...
    if not minesweeper_board.board[row][col].revealed:
        ...
        if minesweeper_board.board[row][col].flag_planted:
            return [Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2, 
                    (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2), 
                    "images/flag.png", int(tile_size / 2), int(tile_size / 2))]

        # else, draw nothing
        else:
            return [Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2, 
                    (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2), 
                    "images/empty.png", int(tile_size / 2), int(tile_size / 2))]
    ...
    if minesweeper_board.board[row][col].type == Tile.MINE:
        return [Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2, 
                (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2), 
                "images/bomb.png", int(tile_size / 2), int(tile_size / 2))]
    ...
    if isinstance(minesweeper_board.board[row][col].value, int):
        ...
        if minesweeper_board.board[row][col].value >= 0:
            ...
            return [Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2, 
                    (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2), 
                    image_name, int(tile_size / 2), int(tile_size / 2))]
        
        # if it's negative, draw a minus sign before the number
        else:
            images = []

            images.append(Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2 - tile_size / 4, 
                        (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2), 
                        "images/minus.png", int(tile_size / 6), int(tile_size / 3)))
            
            image_name = "images/" + str(abs(minesweeper_board.board[row][col].value)) + ".png"
            images.append(Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2 + tile_size / 8, 
                    (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2), 
                    image_name, int(tile_size / 3), int(tile_size / 2)))
    ...
    # else, we can assume the value is a float and draw it to one decimal place
        ...
        if minesweeper_board.board[row][col].value > 0:
            ...
            images.append(Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2 - tile_size / 4, 
                        (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2), 
                        image_name, int(tile_size / 4), int(tile_size / 2)))
            
            # add the decimal point
            images.append(Image(Point((tile_board[row][col].getP1().getX() + tile_board[row][col].getP2().getX()) / 2, 
                        (tile_board[row][col].getP1().getY() + tile_board[row][col].getP2().getY()) / 2 + tile_size / 6), 
                        "images/dot.png", int(tile_size / 6), int(tile_size / 6)))
...
while True:
    run_game(4, 4, 5, 2, difficulty="hard")